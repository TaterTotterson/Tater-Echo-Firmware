...
```
raw 9ch S24_3LE → beamformer + fixed mic gain (micGainDb, applied to 24-bit samples) → mono S16_LE → [AEC + residual suppressor] → 80Hz high-pass → [VAD gate] → [AGC] → /data WebSocket
```
...
- **AEC** (`internal/aec/`) — speexdsp echo canceller (vendored C, SpeexDSP-1.2.1), whole mic path including the wake stream; far-end reference tapped at the speaker ALSA write (every period incl. silence), delayed by `aecDelayMs` — **keep 0**: the mic side's 160ms batch reads absorb the speaker's output latency, and higher values make the echo non-causal (zero cancellation). The mic ALSA ring is only 160ms deep, so >160ms capture stalls silently lose whole batches (~every 20–30s in steady state, load-correlated); an occupancy governor trims the resulting reference backlog **without resetting the filter** — the trim restores the alignment the filter converged against, and the reset that used to live there thrashed convergence to ≤5dB (the v2.7.8 fix). `[aec] att=`/`far:` telemetry logs ~1/s during playback; `[mic] clock/stall` lines track capture loss. `far:` carries `rms`, `mean` and `peak` — **rms alone cannot tell audio from a constant offset**, since both read high, and that ambiguity cost an evening on #117 where the device was writing rms≈4000 to a codec while every speaker stayed silent. `mean≈±rms` with a small peak-to-peak is a DC offset; `mean≈0` with peak well above rms is real audio and the fault is downstream. Note this tap sits after the (L+R)/2 downmix and 3:1 decimation, so DC survives intact but `peak` is mildly smoothed — read it as a floor. It reports only while `aecEnabled`, so a diagnosis that needs it must not have AEC turned off. Default on with barge-in; ~14dB per response, held across turns
  ...
  **Echo state follows the selected physical microphone.** Seven Speex states
  are retained (ch0–ch6) and only the active mic's state is processed, so a
  beam switch no longer applies the centre mic's learned acoustic path to a
  perimeter mic. On selection, stale signal history is cleared while the
  learned filter coefficients remain intact, preventing an old reply tail
  from leaking into the first new frame. This multiplies filter memory, not
  steady-state AEC CPU; hardware-reference state is loaded/saved per mic. A conservative residual
  suppressor can remove at most 6dB and only while the post-AEC output remains
  strongly correlated with the far end. `[aec] att=` now includes three-band
  attenuation, active mic path, residual-suppression and double-talk counts.
...
- **High-pass** (`internal/processor/`) — an in-place 80Hz DC/rumble blocker runs on every stream after AEC and before VAD/AGC. Its state resets with the stream.
- **AGC** (`internal/processor/`) — lock_mic turns only; release is frozen during silence (RMS speech flag), preventing noise floor amplification. Attack/release coefficients are scaled by sample count, so one 2560-sample hardware batch behaves like five 512-sample periods. Processing is in place. (Device-side RNNoise NS was removed 2026-07-12 — noise suppression is controller-side now: `em_ns.py`/DTLN on the ASR-bound stream, per-device `nsAsr` flag)
- **VAD** (lock_mic turns only) runs after AEC/high-pass and before AGC; opens gate after `VAD_SPEECH_MS` of speech, closes after `VAD_SILENCE_MS` of silence, then sends an end-of-speech sentinel
