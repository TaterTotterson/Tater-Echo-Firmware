...
def build(version: str, target: str, output: Path) -> list[Path]:
    ...
    write_json(release_manifest, {
        ...
        "targets": {
            target: {
                **targets[target],
                "artifacts": {
                    "factory": {"name": archive.name, **metadata(archive)},
                    "ota": {"name": ota.name, **metadata(ota)},
                },
            }
        },
    })
    ...
    sums.write_text("".join(
        f"{digest(path)}  {path.name}\n" for path in (archive, ota, release_manifest)))
    return [archive, ota, release_manifest, sums]
